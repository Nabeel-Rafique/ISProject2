...
if ($email != false && $password != false) {
    $sql = "SELECT * FROM hotable WHERE email = '$email'";
    ...
    if ($run_Sql) {
        ...
        if ($status == "verified") {
            if ($code != 0) {
                header('Location: horesetcode.php');
            }
        } else {
            header('Location: ho-otp.php');
        }
    }
} else {
...
<h2 style="font-size: 50px; text-align: center;">Welcome: <?php echo $fetch_info['name'] ?></h2>
...
<br>
                <a href="Postnews.php" class="btn btn-primary" style="display: inline-block;">Post News</a>
                <a href="Postednewshome.php" class="btn btn-primary" style="display: inline-block;">View Posted News</a>
<br>
<br>
                    </form>
